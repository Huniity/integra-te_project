...
convert-clean: ## Convert PNG/JPG assets to WebP and delete originals (runs from srcs/frontend)
	cd srcs/frontend && node -e "const sharp = require('sharp'), fs = require('fs').promises, path = require('path'), dir = 'src/assets'; fs.readdir(dir).then(files => { const targets = files.filter(f => /\.(png|jpe?g)$/i.test(f)); return Promise.all(targets.map(f => { const input = path.join(dir, f), output = path.join(dir, f.replace(/\.(png|jpe?g)$/i, '.webp')); return sharp(input).webp({ quality: 80 }).toFile(output).then(() => fs.unlink(input)).then(() => console.log(`Converted & Deleted: ${f} -> ${path.basename(output)}`)); })); }).then(() => console.log('All assets successfully converted to WebP and originals deleted! 🎉')).catch(console.error);"
